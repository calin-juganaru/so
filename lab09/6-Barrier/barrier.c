...
DWORD CreateThresholdBarrier(THB_OBJECT* pthb, DWORD b_value)
{
	...
	*pthb = hthb;
	...
	DIE(!hthb->hGuard, "CreateMutex()");

	/* TODO - Create Event */
	hthb->hEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
	DIE(!hthb->hEvent, "CreateEvent()");
...
DWORD WaitThresholdBarrier(THB_OBJECT thb)
{
	...
	DIE(dwRet == WAIT_FAILED, "WaitForSingleObject(hGuard)");
	...
	DIE(!bRet, "ReleaseMutex()");

	dwRet = WaitForSingleObject(thb->hEvent, INFINITE);
	DIE(dwRet == WAIT_FAILED, "WaitForSingleObject(hEvent)");
