...
void party(void)
{
	while (1)
    {
        ...
        DWORD dwRet = WaitForSingleObject(dean_mutex, INFINITE);
		DIE(dwRet == WAIT_FAILED, "WaitForSingleObject(dean_mutex)");

		if (dean_in_room)
        {
			dbg_student("o venit decanu'");

			bRet = ReleaseMutex(dean_mutex);
			DIE(!bRet, "ReleaseMutex(dean_mutex)");
...
void enter_room(void)
{
	...
	if (!dean_in_room)
    {
		...
		DIE(dwRet == WAIT_FAILED, "WaitForSingleObject(stud_mutex)");

		++students_at_party;
		dbg_student("apeleaza petrecerea functie");

		bRet = ReleaseMutex(stud_mutex);
		DIE(!bRet, "ReleaseMutex(stud_mutex)");
    ...
    {
		dbg_student("in liniste fugi departe");
