...
static int server(void)
{
	...
	int recv_msgs, recv_count, num_events;
	char msg[MSG_SIZE];
	int i, status, rc, index;
	uint64_t event;
	...
	while (recv_msgs < CLIENT_COUNT)
    {
		...
		num_events = epoll_wait(efd, recv_ev, CLIENT_COUNT - recv_msgs, -1);
		DIE(num_events < 0, "server: epoll_wait");

		for (i = 0; i != num_events; ++i)
        {
			recv_count = read(recv_ev[i].data.fd, msg, MSG_SIZE);
			DIE(recv_count < 0, "server: read");

			msg[recv_count] = '\0';
			printf("server received: %s\n", msg);

			rc = epoll_ctl(efd, EPOLL_CTL_DEL, recv_ev[i].data.fd, recv_ev + i);
			DIE(rc < 0, "server: epoll_ctl(EPOLL_CTL_DEL)");

			rc = close(recv_ev[i].data.fd);
			DIE(rc < 0, "server: close");
		}
...
int main(void)
{
	...
	DIE(event_fd < 0, "main: eventfd failed");
#endif
